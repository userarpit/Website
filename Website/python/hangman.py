...
while count < 10:
    ...
    if letter in random_word:
        all_occurrence_of_letter = [i for i in range(len(random_word)) if random_word.startswith(letter, i)]
        # print(all_occurrence_of_letter)
        
        for index in all_occurrence_of_letter:
            guess_word[index] = letter
        print(f"Guess the word (of length {len(random_word)}) :","".join(guess_word))
